// mapboxgl.accessToken = 'pk.eyJ1IjoibWRhcnRpYyIsImEiOiJjaXM3aGtkenQwMDFsMzBzNGlqMDRjc2UzIn0.2NH0oM9igmUQ0fpg6O5TqQ';
...
var style =
{
    ...
    "sources": {
        "db_name_fr": {
            "type": "vector",
            "tiles": [
                "http://vector-tiles-generator.makina-corpus.net:80/landuse_overlay+landuse+waterway+water+aeroway+building+road+admin+water_label+poi_label+rail_station_label+airport_label+road_label+place_label+marine_label+country_label/{z}/{x}/{y}.pbf"
            ],
            "maxzoom": 14,
            "minzoom": 0
        }
...
map.on('mousemove', function(e) {
    ...
    popup.setLngLat(feature.geometry.coordinates)
        .setHTML("hey " + feature.properties.description)
        .addTo(map);
});
*/
...
var searchHistory = function() {
  ...
  document.getElementById('button').innerHTML = "Merci de patienter... recherche en cours";
  OverpassAPI.get(map.getBounds(), document.getElementById('start').value)
    .then((geojson) => {

        document.getElementById('button').disabled = false;
        document.getElementById('button').innerHTML = "Cherchez les contributions !";


        if (map.getLayer('polygon') !== undefined)
            map.removeLayer('polygon')
        if (map.getLayer('line') !== undefined)
            map.removeLayer('line')
        if (map.getLayer('point') !== undefined)
            map.removeLayer('point')

        if (map.getSource('osm-data-polygon') !== undefined)
            map.removeSource('osm-data-polygon')
        if (map.getSource('osm-data-line') !== undefined)
            map.removeSource('osm-data-line')
        if (map.getSource('osm-data-point') !== undefined)
            map.removeSource('osm-data-point')
      ...
      map.addSource('osm-data-polygon', {
         type: 'geojson',
         data: sourcePolygon
      });

      map.addSource('osm-data-line', {
         type: 'geojson',
         data: sourceLineString
      });

      map.addSource('osm-data-point', {
         type: 'geojson',
         data: sourcePoint
      });
...
map.on('click', function (e) {
    ...
    var feature = features[0],
        ...
        description =
            `<b>Details</b>
            <ul>
              <li><b>Auteur : </b>` + meta.user + `</li>
              <li><b>Version : </b>` + meta.version + `</li>
              <li><b>Date : </b>` + meta.timestamp + `</li>
              <li><b>Tags : </b>` + feature.properties.tags + `</li>
            </ul>`,
